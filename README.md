...
- **スレッド内 `/tips`**: 他ユーザーが補足解説するとき、先頭に `/tips` を付けると FEbot は無反応（RAG・採点・返信なし）。例: `/tips この用語は別の観点から…`
- **小ネタ（固定応答）**: メンションまたは DM で送る（英字は大文字小文字を区別しない）。`$name`（開発者一覧）、`Hey!siri`、`OK,Google`、`"w草w"`、`鴨うまそうっすね` など。一覧は `/fe-help` 参照。
- **DM**: メンション不要。上記キーワードと RAG 質問が同様に使える。
- **コマンド**（Slack 純正 Slash Command 登録は不要。チャンネルは `@ボット /…`、スレッド内・DM は `/…` のみ）:
  - `/fe-help` — ヘルプ表示
  - `/fe-quiz` — Supabase から問題を出題（引数なしで全問ランダム）。例:
    - `/fe-quiz 科目A` または `/fe-quiz a` → 科目A（知識問題）から出題
    - `/fe-quiz 科目B` または `/fe-quiz b` → 科目B（アルゴリズム）から出題
    - `/fe-quiz ネットワーク` など → 分野・タグで絞り込み
  - `/fe-format-test` — Block Kit 表示の目視テスト（AI・RAG 不要）
