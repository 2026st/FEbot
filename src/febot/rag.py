...
【出典】には、回答で実際に根拠とした引用元の名前（URLなど）だけを箇条書きにしてください。根拠に使っていない情報は書かないでください。根拠が参照抜粋にない場合は【出典】に「なし」とだけ書いてください。"""
...
class RagEngine:
    ...
    def answer(self, user_id: str, question: str) -> RagAnswer | None:
        ...
        # 辞書の型ヒントを追加し、Chromaフォールバック時のエラーを防ぐ構造に修正
        url_cache: dict[str, str] = {}

        for doc, src, _meta in picked[:vec_cap]:
            display_source = src  # デフォルトはファイル名

            if self._storage:
                # Supabase接続時のみURLの抽出を試みる
                if src not in url_cache:
                    full_doc = self._storage.get_document_by_source(src)
                    if full_doc and full_doc.get("content"):
                        content = full_doc["content"]
                        # 本文から最初のURLを抽出（余計な括弧や引用符を巻き込まないように正規表現を強化）
                        url_match = re.search(r'(https?://[^\s)\]"\'<>]+)', content)
                        url_cache[src] = url_match.group(1) if url_match else src
                    else:
                        url_cache[src] = src
                
                # キャッシュからURL（または見つからなかった場合のファイル名）を取り出す
                display_source = url_cache[src]

            excerpt = doc.strip() if doc else ""
            if len(excerpt) > 700:
                excerpt = excerpt[:700] + "…"
            
            parts.append(f"### {display_source}\n{excerpt}")
